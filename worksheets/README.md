...
자음 친구들은 두세 명씩 묶어 `../lessons/consonants/lesson-*` 폴더로 관리한다. 현재 자음 학습지 범위는 1레슨 ㄱ/ㄴ부터 6레슨 ㅋ/ㅌ/ㅍ까지다. 마지막 ㅋ/ㅌ/ㅍ 묶음은 분량을 낮추기 위해 `6-A` 캐릭터 만나기와 `6-B` 첫소리 찾기로 나눈다. 모음 전환은 `../lessons/vowels/lesson-*` 폴더에서 시작하며, 첫 레슨은 아아 아기가 아아 나뭇가지를 만나 `아` 소리를 찾는 2쪽 학습지다. 각 자음 폴더에는 다음 파일이 들어간다.
